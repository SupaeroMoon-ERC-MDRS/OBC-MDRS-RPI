...
class RemoteComms(Node):
    def __init__(
        self, protocol: str = "comms.dbc"
    ):  # string is placeholder to be replaced with actual path on device
        ...
        self.cmd_arm_motion_pub = self.create_publisher(
            msg_type=Quaternion, topic="/cmd_move_arm", qos_profile=10
        )
        self.cmd_arm_grip_pub = self.create_publisher(
            msg_type=Bool, topic="/cmd_grip_arm", qos_profile=10
        )
        ...
        if self.res != 0:
            self.get_logger().info(
                "Failed to parse UDPCAN protocol, error code:", self.res
            )
            """Do we need some sort of exit clause here for the error? - ask Em or En when they are freer"""

        self.res = self.nh.init()
        if self.res != 0:
            self.get_logger().info("nh Failed to init, error code:", self.res)

        self.res = self.nh.start()
        if self.res != 0:
            self.get_logger().info("Failed to start thread, error code:", self.res)

        self.remote = (
            self.nh.getRemoteControl()
        )  # This creates the higher order structure that contains the data we need to access - MessageWrapper equivalent, I think
        self.data = (
            RemoteControl()
        )  # object to eventually store the message data when accessed
    ...
    def remote_input(self):
        ...
        if self.e_stop:
            ...
            self.res = self.remote.access(
                self.data
            )  # accesses message within remote and puts it into the data object (RemoteControl object)
            if self.res == 0:  # no error code
                self.e_stop = (
                    self.data.e_stop
                )  # bool # overwriting emergency stop variable with actual input
    ...
    def rover_command(self):
        ...
        self.lin_speed = max(
            min(self.lin_speed, self.max_lin_speed), -self.max_lin_speed
        )
        self.ang_speed = max(
            min(self.ang_speed, self.max_ang_speed), -self.max_ang_speed
        )
        ...
        if (
            self.RB
        ):  # normal stop button - values reset to zero before creating and publishing Twist
            self.lin_speed = 0.0
            self.ang_speed = 0.0
    ...
    def emergency_stop(self):
        ...
        grip_cmd = Bool()
        grip_cmd.data = False
        self.cmd_arm_grip_pub.publish(grip_cmd)
